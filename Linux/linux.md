...
4. сменить права на фвйл                `chmod [ugo] [-+] [rwx] filename`
5. сменить владельца 					`chown -R username:groupname ~/path/dir`
6. специальные разрещения просмотр		i`find / -type f -perm -04000`
6.1. 
...
3.  fg #proc		-- вернуть остановившейся просесс
4.  bg #proc		-- продолжить выполнять проунсс в бакграунде
...
указать порт по которому будем соеденяться.
cat ~/.ssh/config
Host hostnamescp
Port 2222
